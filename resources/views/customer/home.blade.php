...
<body>
                ...
                <div class="slides">
                    @foreach ($promos as $post)
                        <div class="slide">
                            <div class="txt">
                                <h3>{{ $post->title }}</h3>
                                <p class="muted" style="color: #333">{{ $post->content }}</p>
    ...
    <section id="catalogue" class="container" aria-label="Catalogue">
        <h2>Nos produits</h2>
        <div class="sub">Filtrez par catégorie, cherchez par nom, consultez les avis, likez et ajoutez au panier.</div>
    ...
    <section id="services" class="container" aria-label="Services">
        <h2>Services</h2>
        <div class="sub">Coaching minceur, nutrition & suivi santé avec des experts.
        </div>
        <div class="grid">
            <div class="service">
                <h3>Coaching minceur</h3>
                <p class="muted">Plan personnalisé 4 semaines • Bilan initial • Suivi hebdomadaire.</p>
                <button class="btn" onclick="window.location.href='mailto: {{env('MAIL_USERNAME')}}'">Prendre
                    rendez-vous</button>
            </div>
            <div class="service">
                <h3>Nutrition</h3>
                <p class="muted">Menus équilibrés • Ajustés à vos préférences et contraintes.</p>
                <button class="btn" onclick="window.location.href='mailto: {{env('MAIL_USERNAME')}}'">Prendre
                    rendez-vous</button>
            </div>
            <div class="service">
                <h3>Suivi santé</h3>
                <p class="muted">Habitudes de vie • Sommeil • Activité physique douce.</p>
                <button class="btn" onclick="window.location.href='mailto: {{env('MAIL_USERNAME')}}'">Prendre rendez-vous</button>
            </div>
            <div class="service">
                <h3>Ateliers</h3>
                <p class="muted">Ateliers collectifs (hydratation, nutrition, mouvement).</p>
                <button class="btn" onclick="window.location.href='mailto: {{env('MAIL_USERNAME')}}'">En savoir plus</button>
            </div>
        </div>
    </section>
...
@include('layout.footer')
    ...
    <div id="drawer" class="drawer" aria-hidden="true">
        ...
        <aside class="panel">
            ...
            <div style="border-top:1px solid #e5e7eb;padding-top:10px">
                ...
                <div class="muted" style="margin:8px 0">Paiement :</div>
                <div class="flex" style="flex-wrap:wrap;gap:8px">
                    <label class="chip"><input type="radio" name="pay" value="OM" checked> Orange Money</label>
                    <label class="chip"><input type="radio" name="pay" value="MoMo"> MTN MoMo</label>
                    <label class="chip"><input type="radio" name="pay" value="Card"> Carte bancaire</label>
                </div>
    ...
    <div id="commandeModal"
        ...
        <!-- Contenu du modal -->
        <div
            style="background:#fff; width:80%; max-width:900px; margin:50px auto; padding:20px; border-radius:8px; position:relative;">
            <h3 style="margin-bottom:15px;">Liste des commandes</h3>

            <!-- Bouton fermer -->
            <span onclick="closeModal()"
                style="position:absolute; top:10px; right:15px; font-size:20px; cursor:pointer; color:#e74c3c;">
                &times;
            </span>

            <!-- Tableau -->
            <div style="overflow:auto; max-height:400px; border:1px solid #ccc; border-radius:5px;">
                <table style="width:100%; border-collapse:collapse; text-align:center;">
                    <thead>
                        <tr style="background:#333; color:#fff;">
                            <th style="padding:10px; border:1px solid #ccc;">ID</th>
                            <th style="padding:10px; border:1px solid #ccc;">Produit (Quantite)</th>
                    ...
                    <tbody>
                        ...
                        @empty
                            <tr>
                                <td colspan="6" style="padding:10px; border:1px solid #ccc;">Aucune commande trouvée</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    <script>
        ...
        const PRODUCTS = [
            @foreach ($products as $product)
                    ...
                    images: ['{{ asset('images/' . $product->image2) }}', '{{ asset('images/' . $product->image3) }}'],
                    category: '{{ $product->category }}', likes: 120, stars: 4.5
                },
            @endforeach
        ];

        const REVIEWS = [
            { productId: 'prod1', user: 'Alice D.', rating: 5, comment: 'Excellent thé, je me sens plus légère !', date: '2025-09-20' },
            { productId: 'prod1', user: 'Bob M.', rating: 4, comment: 'Bon goût, aide à la digestion.', date: '2025-09-18' },
            { productId: 'prod2', user: 'Carole L.', rating: 5, comment: 'Vraiment efficace pour brûler les graisses.', date: '2025-09-22' },
            { productId: 'prod3', user: 'David P.', rating: 5, comment: 'Shaker très pratique et facile à nettoyer.', date: '2025-09-15' },
            { productId: 'prod4', user: 'Eve R.', rating: 4, comment: 'Le pack est complet, j\'ai déjà vu des résultats.', date: '2025-09-21' },
        ];
        ...
        function renderCartItems() {
            ...
            if (cart.length === 0) {
                cartItemsContainer.innerHTML = '<p class="muted" style="text-align:center;margin-top:20px;">Votre panier est vide.</p>';
            } else {
                cart.forEach(item => {
        ...
        document.getElementById("checkoutBtn").addEventListener("click", function () {
            fetch("/profil/{{ $user->slug }}/addOrder", {
                method: "POST",
                headers: {
                ...
                body: JSON.stringify({
                    cart: cart // ton tableau JS du panier
                })
            })
        ...
        function renderSimilarProducts(currentProductId) {
            ...
            const similar = PRODUCTS.filter(p => p.category === currentProduct.category && p.id !== currentProductId).slice(0, 3);
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            openCartBtn.addEventListener('click', (e) => { e.stopPropagation(); toggleCart(true); });
            openCartBtn2.addEventListener('click', (e) => { e.stopPropagation(); toggleCart(true); });
            ...
            productDialog.addEventListener('click', (e) => {
                if (e.target.matches('.tabs button')) {
                    productDialog.querySelectorAll('.tabs button').forEach(btn => btn.classList.remove('active'));
                    e.target.classList.add('active');
                    const product = PRODUCTS.find(p => p.id === pmMain.dataset.productId); // Need to store product ID in pmMain
                    if (product) {
